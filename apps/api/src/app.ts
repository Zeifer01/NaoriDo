...
import { resolveUploadFilePath } from "./lib/r2.js";
...
app.get("/uploads/*", async (c) => {
  ...
  if (!key) {
    return c.json(
      { success: false, error: { code: "NOT_FOUND", message: "Arquivo não encontrado" } },
      404,
    );
  }
  ...
  try {
    ...
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: "Arquivo inválido" } },
      400,
    );
  }
});
...
app.notFound((c) =>
  c.json(
    { success: false, error: { code: "NOT_FOUND", message: "Rota não encontrada" } },
    404,
  ),
);
