...
customer.get("/:branchSlug/:tableCode/menu", async (c) => {
  ...
  if (!table) {
    return c.json(
      { success: false, error: { code: "NOT_FOUND", message: "Mesa não encontrada" } },
      404,
    );
  }
...
customer.post(
  ...
  async (c) => {
    ...
    if (!branch) return c.json({ success: false, error: { code: "NOT_FOUND", message: "Filial não encontrada" } }, 404);
    ...
    if (!table) return c.json({ success: false, error: { code: "NOT_FOUND", message: "Mesa não encontrada" } }, 404);
    ...
    if (pendingSession) {
      return c.json(
        { success: false, error: { code: "SESSION_PENDING", message: "Esta mesa está aguardando aprovação" } },
        409,
      );
    }
      ...
      return c.json(
        {
          ...
          error: {
            ...
            message: "Esta mesa está aguardando aprovação",
          },
        },
        409,
      );
...
const customerAuth = async (c: any, next: any) => {
  ...
  if (!header || !header.startsWith("Bearer ")) {
    return c.json(
      { success: false, error: { code: "UNAUTHORIZED", message: "Token obrigatório" } },
      401,
    );
  }
  ...
  try {
    ...
    if ((payload as any).role !== "customer") {
      return c.json(
        { success: false, error: { code: "FORBIDDEN", message: "Apenas clientes" } },
        403,
      );
    }
    c.set("user", payload);
...
const requireActiveSession = async (c: any, next: any) => {
  ...
  if (!session) {
    return c.json(
      { success: false, error: { code: "SESSION_ENDED", message: "Sua sessão foi encerrada" } },
      403,
    );
  }
...
customer.post(
  ...
  async (c) => {
    ...
    if (!coupon) {
      return c.json(
        { success: false, error: { code: "NOT_FOUND", message: "Cupom não encontrado" } },
        404,
      );
    }
    ...
    if (coupon.status !== "active") {
      return c.json(
        { success: false, error: { code: "INVALID", message: "O cupom não está ativo" } },
        400,
      );
    }
    ...
    if (coupon.starts_at && now < coupon.starts_at) {
      return c.json(
        { success: false, error: { code: "INVALID", message: "O cupom ainda não está vigente" } },
        400,
      );
    }
    if (coupon.expires_at && now > coupon.expires_at) {
      return c.json(
        { success: false, error: { code: "INVALID", message: "O cupom expirou" } },
        400,
      );
    }
    if (coupon.max_uses_total && coupon.current_uses >= coupon.max_uses_total) {
      return c.json(
        { success: false, error: { code: "INVALID", message: "O cupom atingiu o limite de usos" } },
        400,
      );
    }
...
// POST /orders/:id/items - Customer adds item to own pending order (Fase C)
customer.post(
  "/orders/:id/items",
  customerAuth,
  requireActiveSession,
  ...
  async (c) => {
    ...
    // Find the order and verify it belongs to the customer's active session
    const [session] = await db
    ...
    if (!session) {
      return c.json(
        { success: false, error: { code: "FORBIDDEN", message: "Sessão expirou" } },
        403,
      );
    }
    ...
    // Cliente só pode adicionar antes da cozinha começar (mais restritivo que staff)
      ...
      return c.json(
        {
          ...
          error: {
            code: "CONFLICT",
            message: "O pedido já está sendo preparado. Faça um novo pedido.",
          },
        },
        409,
      );
    ...
    try {
      ...
      await wsManager.publish(`branch:${branchId}`, wsPayload);
      await wsManager.publish(`branch:${branchId}:kitchen`, wsPayload);
      await wsManager.publish(`session:${session.id}`, wsPayload);
...
customer.post(
  ...
  async (c) => {
    ...
    if (nonPendingItem) {
      return c.json(
        { success: false, error: { code: "CONFLICT", message: "O pedido já está sendo preparado" } },
        409,
      );
    }
    ...
    return c.json({ success: true, data: { message: "Pedido cancelado com sucesso" } });
  },
);
...
customer.post(
  ...
  async (c) => {
    ...
    if (tableSessionId !== activeSession.id) {
      return c.json(
        { success: false, error: { code: "FORBIDDEN", message: "Sessão inválida para esta mesa" } },
        403,
      );
    }
      ...
      return c.json(
        {
          ...
          error: {
            ...
            message: `Aguarde ${retryAfterSec}s antes de enviar esta solicitação novamente`,
          },
          data: { retryAfterSec },
        },
        429,
...
customer.post(
  ...
  async (c) => {
    ...
    if (!customerId) {
      return c.json(
        { success: false, error: { code: "BAD_REQUEST", message: "Você não possui conta de fidelidade" } },
        400,
      );
    }
    ...
    if (!enrollment) {
      return c.json(
        { success: false, error: { code: "BAD_REQUEST", message: "Você não está inscrito no programa de fidelidade" } },
        400,
      );
    }
    ...
    try {
      ...
      if (message === "REWARD_NOT_FOUND") {
        return c.json({ success: false, error: { code: "NOT_FOUND", message: "Recompensa não encontrada" } }, 404);
      }
      if (message === "INSUFFICIENT_POINTS") {
        return c.json({ success: false, error: { code: "BAD_REQUEST", message: "Pontos insuficientes" } }, 400);
      }
      if (message === "PROGRAM_MISMATCH") {
        return c.json({ success: false, error: { code: "BAD_REQUEST", message: "A recompensa não pertence ao seu programa" } }, 400);
      }
      throw err;
    }
  },
