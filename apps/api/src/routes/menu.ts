...
menu.delete(
  ...
  async (c) => {
    ...
    const items = await db
      ...
      .where(
        and(
          eq(schema.menuItems.category_id, id),
    ...
    if (items.length > 0) {
      ...
      const [orderedItem] = await db
        ...
        .where(inArray(schema.orderItems.menu_item_id, itemIds))
        .limit(1);
        ...
        return c.json(
          {
            ...
            error: {
              ...
              message:
                "Não é possível excluir esta categoria porque há produtos que já foram usados em pedidos. Desative a categoria em vez de excluí-la.",
            },
          },
          409,
        );
    ...
    return c.json({ success: true, data: { message: "Categoria excluída" } });
  },
);
...
// PATCH /items/reorder — must come before /items/:id
...
menu.delete(
  ...
  async (c) => {
      ...
      return c.json(
        {
          ...
          error: {
            code: "ITEM_IN_USE",
            message:
              "Não é possível excluir este produto porque ele já foi usado em pedidos. Desative a disponibilidade em vez de excluí-lo.",
          },
        },
        409,
      );
    ...
    return c.json({ success: true, data: { message: "Produto excluído" } });
  },
);
