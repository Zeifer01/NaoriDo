...
interface CustomerState {
  ...
  /** When set, /cart will ADD items to this existing order instead of creating a new one. */
  ...
  setAddToOrderId: (orderId: string | null) => void;
  clear: () => void;
}
...
export const useCustomerStore = create<CustomerState>((set) => ({
  ...
  addToOrderId: getItem("customer_add_to_order_id"),
  setSession: (data) => {
    set({
      token: data.token,
  ...
  setAddToOrderId: (orderId) => {
    set({ addToOrderId: orderId });
    if (isBrowser) {
      if (orderId) {
        sessionStorage.setItem("customer_add_to_order_id", orderId);
      } else {
        sessionStorage.removeItem("customer_add_to_order_id");
      }
    }
  },
  clear: () => {
