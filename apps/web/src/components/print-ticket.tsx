...
<body>
  ...
  ${data.notes ? `<div class="divider"></div><div style="font-size:11px;">Obs: ${data.notes}</div>` : ""}
  <div class="divider"></div>
  <div class="center" style="font-size:10px;margin-top:4px;">*** FIM ***</div>
</body>
</html>`;
}
...
function buildReceiptTicketHtml(data: ReceiptTicketData): string {
  ...
  let docTitle = "RECIBO DE VENDA";
  let docInfoHtml = "";
  if (data.docType === "boleta_electronica") {
    docTitle = "RECIBO DE VENDA ELETRÔNICO";
    if (data.docNumber) {
      docInfoHtml = `<div>CPF: ${data.docNumber}</div>`;
    }
    if (data.customerName) {
      docInfoHtml += `<div>Cliente: ${data.customerName}</div>`;
    }
  } else if (data.docType === "factura") {
    docTitle = "NOTA FISCAL";
    if (data.docNumber) {
      docInfoHtml = `<div>CNPJ: ${data.docNumber}</div>`;
    }
    if (data.docHolderName) {
      docInfoHtml += `<div>Razão Social: ${data.docHolderName}</div>`;
    }
  } else {
...
<body>
  ...
  ${data.ruc ? `<div class="center" style="font-size:10px;">CNPJ: ${data.ruc}</div>` : ""}
  ...
  <div class="divider"></div>
  ${data.paymentMethod ? `<div>Método de pagamento: ${methodLabels[data.paymentMethod] || data.paymentMethod}</div>` : ""}
  <div class="divider"></div>
  <div class="center" style="font-size:10px;margin-top:4px;">Obrigado pela preferência</div>
  <div class="center" style="font-size:9px;">*** FIM ***</div>
</body>
</html>`;
}
