...
function timeAgo(ts: number): string {
  ...
  if (seconds < 60) return "há um momento";
  ...
  const handleWsMessage = useCallback(
    (msg: WsMessage) => {
      ...
      if (msg.type === "table:call_waiter") {
        addNotification({
          type: "call_waiter",
          message: `Mesa ${payload.tableNumber}: ${payload.customerName || "Cliente"} solicita o garçom`,
        ...
        addNotification({
          type: "request_bill",
          message: `Mesa ${payload.tableNumber}: ${payload.customerName || "Cliente"} solicita a conta`,
          tableNumber: payload.tableNumber,
          tableId: payload.tableId,
          timestamp: msg.timestamp,
        });
        ...
        addNotification({
          ...
          message: `Mesa ${payload.tableNumber}: Nova conexão de ${payload.customerName || "cliente"}`,
          tableNumber: payload.tableNumber,
          tableId: payload.tableId,
          timestamp: msg.timestamp,
        });
