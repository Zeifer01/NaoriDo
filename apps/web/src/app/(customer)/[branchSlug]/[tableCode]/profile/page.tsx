...
export default function ProfilePage({
  ...
  const handleConfirmRedeem = async () => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error?.message || "Erro ao resgatar");
      }
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="p-4 space-y-3">
            ...
            {loyalty.next_tier && (
              <div className="space-y-1">
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Progresso para {loyalty.next_tier.name}</span>
                  <span>
            ...
            <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
              ...
              <span>Ganhe pontos a cada pedido</span>
            </div>
          </CardContent>
        </Card>
      )}
                  ...
                  <div className="shrink-0 ml-3 flex flex-col items-end gap-1">
                    ...
                    {canRedeem ? (
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        Faltam {(reward.points_cost - loyalty.points_balance).toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {r.discount_type === "percentage"
                      ? `${r.discount_value}% de de desconto`
                      : `${formatCurrency(r.discount_value)} de de desconto`}
                  </p>
                </div>
            ...
            <p className="text-[10px] text-muted-foreground text-center">
              Será aplicado automaticamente no seu próximo pedido a partir do carrinho
            </p>
          </CardContent>
        </Card>
      )}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    ...
                    {coupon.expires_at && (
                      <> · Vence {new Date(coupon.expires_at).toLocaleDateString("pt-BR")}</>
                    )}
                  </p>
                </div>
              </div>
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Você ainda não tem atividade. Faça um pedido para começar a acumular pontos.
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedReward && loyalty && (
                <>
                  Resgatar <strong>{selectedReward.name}</strong> por{" "}
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              {redeemResult && (
                <>
                  {redeemResult.discount.type === "percentage"
                    ? `${redeemResult.discount.value}% de desconto`
                    : `${formatCurrency(redeemResult.discount.value)} de desconto`}
                  <br />
              ...
              <strong className="text-foreground text-sm mt-2 block">
                Vá ao carrinho para aplicar o desconto no seu próximo pedido
              </strong>
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
