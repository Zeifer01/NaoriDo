...
const steps = [
  ...
  { key: "ready", label: "Pronto", icon: UtensilsCrossed },
...
export default function OrderStatusPage({
  ...
  const handleTableAction = useCallback(async (action: TableAction) => {
    ...
    if (remainingMs > 0) {
      ...
      setActionNotice({
        kind: "error",
        message: `Aguarde ${remainingSeconds}s antes de enviar esta solicitação novamente.`,
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !result.success) {
        ...
        throw new Error(result.error?.message || "Não foi possível enviar a solicitação");
      }
      ...
      const successMessage =
        action === "request_bill"
          ? "Sua solicitação de conta foi enviada ao restaurante."
          : "Sua solicitação de garçom foi enviada ao restaurante.";
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Não foi possível enviar a solicitação. Intenta nuevamente.";
  ...
  const fetchOrder = useCallback(async () => {
    ...
    if (!orderId || !token) {
      setError("Pedido não encontrado. Tente fazer um novo pedido.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const result = await res.json();
      if (!result.success) {
        throw new Error(result.error?.message || "Erro ao obter o pedido");
      }
  ...
  const handleCancelOrder = useCallback(async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (!result.success) {
        setError(result.error?.message || "Não foi possível cancelar o pedido");
        return;
      }
      await fetchOrder();
    } catch {
  ...
  if (error || !order) {
    return (
      <div className="p-6 mt-12 text-center">
        <p className="text-destructive font-medium mb-4">{error || "Pedido não encontrado"}</p>
    ...
    <div className="p-4 space-y-5">
      ...
      {showConfirmation && (
        <div className="rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-4 text-center animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <p className="font-semibold text-green-800 dark:text-green-300">Pedido recebido</p>
          <p className="text-sm text-green-600 dark:text-green-400">Seu pedido foi enviado para a cozinha</p>
        </div>
      )}
      ...
      <div className="text-center pt-2">
        ...
        <p className="text-muted-foreground mt-1">Pedido #{order.order_number}</p>
      </div>
      ...
      {isCancelled ? (
        <Card>
          <CardContent className="p-5 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Este pedido foi cancelado</p>
          </CardContent>
        </Card>
      ) : (
        /* Stepper */
      ...
      <Link href={`/${branchSlug}/${tableCode}/profile`}>
        <Card className="border-primary/20 bg-primary/5 hover:bg-primary/10 transition-colors cursor-pointer">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <p className="text-sm font-medium flex-1">Ver meus pontos e recompensas</p>
      ...
      {!isCancelled && (
        <div className="flex gap-3">
          ...
          <Link
            href={`/${branchSlug}/${tableCode}/menu`}
            className="flex-1"
            onClick={() => {
              setAddToOrderId(canAddItems ? order.id : null);
            }}
          >
        ...
        <>
            ...
            >
              ...
              {actionLoading === "request_bill"
                ...
                : requestBillCooldownSeconds > 0
                  ? `Tentar novamente em ${requestBillCooldownSeconds}s`
            ...
            >
              ...
              {actionLoading === "call_waiter"
                ...
                : callWaiterCooldownSeconds > 0
                  ? `Tentar novamente em ${callWaiterCooldownSeconds}s`
                  : actionSent.call_waiter
                    ? "Chamar o Garçom (novamente)"
                    : "Chamar o Garçom"}
            </Button>
          </div>
          {actionNotice && (
          ...
          {(requestBillCooldownSeconds > 0 || callWaiterCooldownSeconds > 0) && (
            <p className="text-xs text-muted-foreground">
              Anti-spam ativo: cada solicitação tem 30 segundos de espera.
            </p>
          )}
        </>
      )}
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tem certeza de que deseja cancelar seu pedido? Esta ação não pode ser desfeita.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
