  ...
  if (!item) {
    return (
      <div className="p-6 mt-12 text-center">
        ...
        <p className="font-medium mb-2">Produto não encontrado</p>
        <p className="text-sm text-muted-foreground mb-4">
    ...
    for (const group of modifierGroups) {
      if (group.is_required) {
        ...
        if (sel.length < (group.min_selections || 1)) {
          alert(
            `Selecione pelo menos ${group.min_selections || 1} opção em "${group.name}"`,
          );
          return;
        }
      }
      ...
      <div className="relative w-full h-72 bg-muted overflow-hidden">
        {item.image_url ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-3 bg-muted">
            ...
            <p className="text-sm text-muted-foreground/50">Sem imagem</p>
            <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
          </div>
        )}
