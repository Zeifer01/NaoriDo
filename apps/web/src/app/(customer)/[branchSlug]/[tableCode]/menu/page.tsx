...
export default function CustomerMenuPage({
  ...
  const loadMenu = useCallback(() => {
    ...
    void fetch(`${API_URL}/api/customer/${branchSlug}/${tableCode}/menu`)
      ...
      .then((result) => {
        if (!result.success) {
          setError(result.error?.message || "Erro ao carregar o cardápio");
          setLoading(false);
          return;
        }
  ...
  const handleTableAction = async (action: "request_bill" | "call_waiter") => {
    ...
    try {
      ...
      toast.success(
        action === "request_bill"
          ? "Conta solicitada. A equipe foi notificada."
          : "Garçom solicitado. A equipe foi notificada."
      );
    } catch (err) {
  ...
  if (error || !menuData) {
    return (
      <div className="p-6 mt-12 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4">{error || "Erro ao carregar cardápio"}</p>
                      ...
                      <div className="relative w-full h-40">
                        ...
                        {qty > 0 ? (
                          ...
                          >
                            ...
                            <span className="text-[9px] font-medium leading-none mt-0.5">Adicionar</span>
                          </button>
                        )}
                      </div>
