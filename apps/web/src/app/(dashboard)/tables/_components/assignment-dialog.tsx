  ...
  return (
    <Dialog open={!!table} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Atribuir Garçons - Mesa {table?.number}
          </DialogTitle>
          <DialogDescription>
            Gerencie os garçons atribuídos a esta mesa
          </DialogDescription>
        </DialogHeader>
        {table && (
          <div className="space-y-4">
            {/* Current assignments */}
            <div>
              <Label className="text-sm mb-2 block">Garçons atribuídos</Label>
              {assignments.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">Nenhum garçom atribuído</p>
              ) : (
            ...
            {/* Add waiter */}
            <div>
              <Label className="text-sm mb-2 block">Adicionar garçom</Label>
              <Select
                key={assignKey}
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="Selecionar garçom..." />
                </SelectTrigger>
                <SelectContent>
                  {waiters
