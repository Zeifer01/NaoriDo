...
export default function TablesPage() {
  ...
  const handleWsMessage = useCallback((msg: WsMessage) => {
    ...
    toast.info(
      requestType === "request_bill"
        ? `Mesa ${payload.tableNumber}: ${payload.customerName || "Cliente"} solicita a conta`
        : `Mesa ${payload.tableNumber}: ${payload.customerName || "Cliente"} solicita garçom`
    );
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <PageHeader
        title="Mesas e Espaços"
        description={
          isLoading
            ? "Carregando..."
            : `${counts.available} disponíveis, ${counts.occupied} ocupadas de ${counts.total} mesas`
        }
        actions={
          <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Disponíveis", value: counts.available, color: "text-green-600 dark:text-green-400" },
        ...
        <Card className="border-2 border-amber-300 dark:border-amber-700 bg-amber-50/50 dark:bg-amber-950/20">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Solicitudes pendentes ({pendingSessions.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent className="p-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium">
                Solicitudes ativas: {requestSummary.total}
              </p>
              <Badge variant="outline">Conta: {requestSummary.requestBillCount}</Badge>
              <Badge variant="outline">Garçom: {requestSummary.callWaiterCount}</Badge>
            </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex items-center gap-2 overflow-x-auto">
          <TabsList>
            ...
            <TabsTrigger value="unassigned">Sem espaço</TabsTrigger>
          </TabsList>
        </div>
      ...
      <ConfirmDialog
        ...
        description={
          deleteConfirm?.type === "space"
            ? `Tem certeza de que deseja excluir o espaço "${deleteConfirm.name}"? Só pode ser excluído se não tiver mesas atribuídas.`
            : `Tem certeza de que deseja excluir "${deleteConfirm?.name}"? Esta ação não pode ser desfeita.`
        }
