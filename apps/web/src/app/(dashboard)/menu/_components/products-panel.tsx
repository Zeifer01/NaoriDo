...
export function ProductsPainel({
  categories,
  menuItems,
  allModifierGroups,
  isLoading,
  ...
  const activeCategoryName =
    selectedCategoryId === "all"
      ? "Todos os produtos"
      : categoryList.find((c: any) => c.id === selectedCategoryId)?.name ?? "Produtos";
    ...
    <div className="flex gap-6">
        ...
        {visibleItems.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-border rounded-lg">
            ...
            <p className="text-sm text-muted-foreground mb-3">
              {search ? "Nenhum produto encontrado" : "Sem produtos nesta categoria"}
            </p>
            {!search && (
              <Button
                variant="outline"
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          title={`Excluir ${confirmDelete.type === "category" ? "categoria" : "produto"}`}
          description={`Tem certeza de que deseja excluir "${confirmDelete.name}"? Esta ação não pode ser desfeita.`}
