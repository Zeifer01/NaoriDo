...
export function TierDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isEdit ? "Editar Nível" : "Criar Nível"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="tier-name">Nome do nível</Label>
            <Input id="tier-name" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} required placeholder="Ex: Diamante" />
          </div>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={createTier.isPending || updateTier.isPending || !form.name}>
              {(createTier.isPending || updateTier.isPending) ? "Salvando..." : isEdit ? "Salvar" : "Criar Nível"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
