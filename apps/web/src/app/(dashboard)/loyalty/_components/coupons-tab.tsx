...
const couponTypeLabels: Record<string, string> = {
  ...
  fixed: "Valor fixo",
  item_free: "Item grátis",
  item_discount: "Desconto no item",
  category_discount: "Desconto na categoria",
...
const couponStatusLabels: Record<string, { label: string; color: string }> = {
  ...
  inactive: { label: "Inativo", color: "bg-gray-100 text-gray-800 dark:bg-gray-700/40 dark:text-gray-300" },
...
function AssignCouponDialog({
  ...
  function handleAssign() {
    ...
    assignCoupon.mutate(
      { couponId: coupon.id, customerIds: selectedIds },
      {
        onSuccess: () => {
          setSelectedIds([]);
          toast.success(`Cupom atribuído a ${selectedIds.length} cliente(s)`);
          onOpenChange(false);
        },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Atribuir Cupom: {coupon?.code}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          {assignments.length > 0 && (
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2">
                Já atribuídos ({assignments.length}):
              </p>
          ...
          <div className="max-h-60 overflow-y-auto border border-border rounded-lg">
            {customersLoading ? (
              ...
              <div className="p-4 text-center text-sm text-muted-foreground">
                {debouncedSearch ? "Sem resultados" : "Nenhum cliente"}
              </div>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {cust.phone || cust.email || "Sem contato"}
                      </p>
                    </div>
                    {alreadyAssigned ? (
...
export function CouponsTab() {
  ...
  function handleToggleStatus(coupon: any) {
    ...
    updateCoupon.mutate(
      ...
      {
        onSuccess: () => toast.success(`Cupom ${newStatus === "active" ? "ativado" : "desativado"}`),
        onError: (err) => toast.error(`Error: ${(err as Error).message}`),
      },
    );
  }
  ...
  function handleDelete() {
    ...
    deleteCoupon.mutate(deleteConfirm.id, {
      onSuccess: () => { setDeleteConfirm(null); toast.success("Cupom excluído"); },
      onError: (err) => toast.error(`Error: ${(err as Error).message}`),
    });
  }
  ...
  function formatCouponValue(coupon: any): string {
    switch (coupon.type) {
      ...
      case "item_free": return "Item grátis";
      case "item_discount": return `${coupon.discount_value}% no item`;
      case "category_discount": return `${coupon.discount_value}% na categoria`;
  ...
  if (error) {
    return (
      <div className="p-4 rounded-lg border border-destructive/50 bg-destructive/10 flex items-center justify-between">
        <p className="text-sm text-destructive">Erro ao carregar cupons: {(error as Error).message}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          ...
          <SelectContent>
            ...
            <SelectItem value="item_free">Item grátis</SelectItem>
            <SelectItem value="item_discount">Desconto no item</SelectItem>
            <SelectItem value="category_discount">Desconto na categoria</SelectItem>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground mb-1">Nenhum cupom criado</p>
            <p className="text-xs text-muted-foreground">Crie cupons de desconto para seus clientes</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <CardContent className="p-5">
                  ...
                  {(coupon.starts_at || coupon.expires_at) && (
                    <div className="mt-2 text-xs text-muted-foreground">
                      {coupon.starts_at && (
                        <span>De: {new Date(coupon.starts_at).toLocaleDateString("pt-BR")} </span>
                      )}
                      {coupon.expires_at && (
      ...
      <ConfirmDialog
        ...
        title="Excluir cupom"
        description={`Tem certeza de que deseja excluir o cupom ${deleteConfirm?.name}?`}
