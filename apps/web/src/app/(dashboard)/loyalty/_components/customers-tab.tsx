  ...
  if (error) {
    return (
      <div className="p-4 rounded-lg border border-destructive/50 bg-destructive/10 flex items-center justify-between">
        <p className="text-sm text-destructive">Erro ao carregar clientes: {(error as Error).message}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmDialog
        ...
        description={`Será excluído "${deleteConfirm?.name}" e todos os seus dados de fidelidade (pontos, transações, cupons). Os pedidos existentes serão mantidos. Esta ação não pode ser desfeita.`}
