    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={handleCreate} disabled={!programId}>
          <Plus className="h-4 w-4 mr-2" />Criar Recompensa
        </Button>
      </div>
      ...
      {!programId && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">Você precisa criar um programa de fidelidade primeiro</p>
          </CardContent>
        </Card>
      )}
      ...
      {rewardsList.length === 0 && programId && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground mb-1">Nenhuma recompensa criada</p>
            <p className="text-xs text-muted-foreground">Crie recompensas para que seus clientes resgatem seus pontos</p>
          </CardContent>
        </Card>
      )}
            ...
            <CardContent className="p-5 pt-4">
              ...
              <div className="mt-4 flex items-center gap-2">
                <Badge variant="secondary" className="text-xs">
                  {reward.discount_type === "percentage" ? `${reward.discount_value}% de desconto` : `${formatCurrency(reward.discount_value)} de desconto`}
                </Badge>
                {reward.is_active ? (
                  <Badge variant="outline" className="text-xs text-green-600 border-green-300 dark:text-green-400 dark:border-green-700">Ativa</Badge>
                ) : (
                  <Badge variant="outline" className="text-xs text-red-600 border-red-300 dark:text-red-400 dark:border-red-700">Inativa</Badge>
                )}
              </div>
      ...
      <ConfirmDialog
        ...
        description="Se a recompensa tiver resgates, será desativada em vez de excluída."
