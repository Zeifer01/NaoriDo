...
export default function StaffPage() {
  ...
  const handleStartShift = async () => {
    try {
      ...
      toast.error(err.message || "Erro ao iniciar turno");
    }
  };
  ...
  const handleEndShift = async (shiftId: string) => {
    try {
      ...
      toast.error(err.message || "Erro ao finalizar turno");
    }
  };
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="p-4 rounded-lg border border-destructive/50 bg-destructive/5 flex items-center justify-between">
          <p className="text-sm text-destructive">Erro ao carregar equipe: {(error as Error).message}</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Staff"
        description={isLoading ? "Carregando..." : `${activeCount} membros ativos`}
        actions={
          <>
            {myActiveShift ? (
              <Button
