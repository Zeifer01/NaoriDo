...
interface NavItem {
  ...
  /** Optional plan feature required for this entry to render in the sidebar. */
  feature?: PlanFeature;
}
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/connections", label: "Conexões", icon: Wifi, feature: "whatsapp" },
...
const roleNavAccess: Record<string, Set<string>> = {
  super_admin: new Set(navGroups.flatMap((g) => g.items.map((i) => i.href))),
  org_admin: new Set(
    navGroups
  ...
  branch_manager: new Set(
    navGroups
      .flatMap((g) => g.items.map((i) => i.href))
      .filter((href) => href !== "/settings" && href !== "/super-admin"),
  ),
        ...
        <div className="h-14 flex items-center gap-3 px-4 border-b border-sidebar-border">
          {orgLogoUrl ? (
            <img
              src={orgLogoUrl}
              alt={orgName}
              className="h-8 w-8 shrink-0 rounded-lg object-cover border border-sidebar-border"
            />
          ) : (
          ...
          {!collapsed ? (
            <div className="flex items-center gap-3 rounded-md px-2 py-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground truncate leading-tight">
                  {user.role === "owner" ? "Proprietário" : user.role === "admin" ? "Administrador" : "Equipe"}
                </p>
              </div>
              <button
        ...
        <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="flex items-center justify-between h-14 px-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-semibold md:hidden flex items-center gap-2">
                {orgLogoUrl && (
                  <img
                    src={orgLogoUrl}
                    alt=""
