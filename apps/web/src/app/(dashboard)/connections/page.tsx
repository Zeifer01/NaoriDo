...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  pending: { label: "Pendente", color: "bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-500/20", icon: Clock },
  active: { label: "Ativa", color: "bg-green-500/15 text-green-700 dark:text-green-400 border-green-500/20", icon: UserCheck },
  completed: { label: "Concluída", color: "bg-muted text-muted-foreground border-border", icon: Check },
  rejected: { label: "Recusada", color: "bg-red-500/15 text-red-700 dark:text-red-400 border-red-500/20", icon: UserX },
};
          ...
          {isLoading ? (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-muted-foreground">Nenhuma sessão {tab === "pending" ? "pendentes" : tab === "active" ? "ativas" : ""}</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-3">
